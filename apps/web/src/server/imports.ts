...
export function getImportTemplates(): ImportTemplateDefinition[] {
  return templates.map((template) => ({ ...template, columns: [...template.columns] }));
}
...
function normalizeOrder(row: RawImportRow, context: ImportContext, seenOrderNos: Set<string>) {
  ...
  const billingMode = requireText(row, "计费方式", errors);
  ...
  const orderStatus = mapOrderStatus(readText(row, "订单状态") || "进行中");
  ...
  return {
    record: {
      ...
      vehicleSourceType: mapVehicleSource(vehicleSourceText),
      billingMode,
      rentalStartDate,
      rentalEndDate,
...
function normalizeRevenue(row: RawImportRow, context: ImportContext) {
  ...
  return {
    record: {
      ...
      receiptDate,
      companyAccount,
      financeReviewStatus: mapFinanceReviewStatus(readText(row, "财务审核状态")),
      revenueKind: mapRevenueKind(readText(row, "收入口径")),
      isCommissionable: parseYesNo(readText(row, "是否参与提成"), true),
      remark: readText(row, "备注"),
      dataSource: "IMPORT"
    },
    errors
...
function normalizeDeposit(row: RawImportRow, context: ImportContext) {
  ...
  return {
    record: {
      ...
      receivedDate,
      refundAmountCents,
      refundStatus: mapRefundStatus(readText(row, "退还状态")),
      remark: readText(row, "备注"),
      dataSource: "IMPORT"
    },
    errors
...
function mapVehicleSource(value: string): "OWNED" | "EXTERNAL" {
  return value.includes("外") || value.toUpperCase() === "EXTERNAL" ? "EXTERNAL" : "OWNED";
}

function mapOrderStatus(value: string): string {
  if (value.includes("完成")) return "COMPLETED";
  if (value.includes("取消")) return "CANCELLED";
  if (value.includes("提交")) return "SUBMITTED";
  return "ACTIVE";
}

function mapFinanceReviewStatus(value: string): FinanceReviewStatus {
  if (value.includes("拒") || value.toUpperCase() === "REJECTED") return "REJECTED";
  if (value.includes("审") || value.toUpperCase() === "APPROVED") return "APPROVED";
  return "PENDING";
}

function mapRevenueKind(value: string): RevenueKind {
  return value.includes("历史") ? "HISTORICAL_RECEIVABLE" : "OWNED_RENT";
}

function mapRefundStatus(value: string): "HELD" | "PARTIALLY_REFUNDED" | "REFUNDED" | "DISPUTED" {
  if (value.includes("争议")) return "DISPUTED";
  if (value.includes("部分")) return "PARTIALLY_REFUNDED";
  if (value.includes("已退")) return "REFUNDED";
  return "HELD";
}

function parseYesNo(value: string, defaultValue: boolean): boolean {
  if (!value) return defaultValue;
  return ["是", "yes", "true", "1", "Y"].includes(value) || value.toUpperCase() === "YES";
}
